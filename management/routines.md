...
## 毎朝調査(ニュースIT・ビジネス・Olise) ― 2026-07-25時点、一時停止中

- **状態**: 一時停止中(`enabled: false`)。毎日開くわけではないことが分かったため停止。設定は残しており、`RemoteTrigger`で`enabled: true`に戻せばいつでも再開可能。週次など別のペースで再開する可能性あり
...
## 宇宙100年史ブログシリーズ(手動実行のみ、不定期)

- `departments/blog/series-space/instructions.md` に従う、日々のニュースとは別枠の連載企画
- 宇宙138億年の歴史を人間の100年の人生に置き換えて語る(1歳=1.38億年、「人生100年時代」に合わせた換算)
- 1話1万字程度のガッツリ深掘り。用語ごとの豆知識(本文中に散りばめる)・次回予告を含む構成。「現在地(シリーズ全体のどのあたりか)」は本文ではなくアイキャッチの年齢メーターで見せる
- ユーザーが「宇宙シリーズ書いて」等と依頼した時にのみ実行。次に書く「年齢」は`departments/blog/series-space/progress.md`で管理する(必ず書く前に確認・書いた後に追記)
- ルール・校閲・アイキャッチ作成は`departments/blog/instructions.md`の一般ルールを継承(アイキャッチには年齢メーターを毎回入れる)
